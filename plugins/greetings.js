...
bot(
	...
	async (message, match) => {
		...
		if (!match && !welcome)
			return await message.sendMessage('*Hai, &mention* \nSilahkan duduk')
		if (!match) {
...
bot(
	...
	async (message, match) => {
		...
		if (!match && !welcome)
			return await message.sendMessage('*Selamat tinggal &mention* jgn lupa celananya dibawa.')
		if (!match) {
			await message.sendMessage(welcome.message)
			return await message.sendMessage(
				await genHydratedButtons(
		...
		if (match == 'on' || match == 'off') {
			if (!welcome)
				return await message.sendMessage('*Selamat tinggal #mention*')
